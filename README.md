...
O padrão econômico é 1 task, RDS Single-AZ e hosts com IPv4 público, **sem SSH e sem entrada pública direta**: só o ALB pode alcançar portas dinâmicas das tasks bridge. Isso elimina o NAT. RDS fica em sub-redes isoladas; S3 usa gateway endpoint. IMDSv2 obrigatório, bloqueio de IMDS no Docker, disco EBS criptografado, gerenciamento por SSM e contêiner sem root, sem capabilities, filesystem somente leitura.

`private_compute=true` move os hosts para sub-redes privadas com **um NAT Gateway e um Elastic IP**. Use esse modo se o SMTP exigir IP de saída fixo e autorize o output `smtp_egress_ip` no Brevo. No modo econômico, o IP muda com substituição/autoscaling; uma lista SMTP de IPs fixa não acompanha essas mudanças. O NAT único tem custo e dependência de uma AZ. `min_tasks=2` e `rds_multi_az=true` aumentam disponibilidade e preço; o padrão não é alta disponibilidade.
...
1. Terraform >=1.10, AWS CLI autenticada e permissão de criação dos recursos. Execute `aws sts get-caller-identity` e confira a conta. Esta entrega fez apenas consultas à AWS, nenhum provisionamento real.
2. Crie um bucket de estado, com nome globalmente único:
   ...
   ```
   O bootstrap tem estado local: guarde cópia criptografada fora do Git. O bucket tem versionamento, bloqueio público, TLS obrigatório e `prevent_destroy`. Seu conteúdo não deve ser removido durante a vida da infraestrutura.
3. Copie `aws/backend.hcl.example` para `aws/backend.hcl`, informe o bucket. O backend usa S3 com `use_lockfile=true`; não precisa DynamoDB. Copie `aws/terraform.tfvars.example` para `aws/terraform.tfvars`. Repositório já preenchido como `JeanRoque0/broto-api-golang`; informe domínio da API, zona pública Route53 existente, site/origens e remetente SMTP. Registre/delegue o domínio antes de validar ACM.
...
- ECR mantém 30 releases e remove untagged após 14 dias. Guarde por mais tempo imagens necessárias à sua política de rollback.
- Logs API/migration retidos por 14 dias; alarmes para memória/disco RDS e ausência de hosts saudáveis. Configure `alarm_email` e confirme assinatura SNS. Não habilitamos logs de acesso ALB/CloudFront para evitar gravar query strings com credenciais temporárias; WAF, Container Insights e RDS Proxy não estão incluídos no custo.
- RDS exporta logs PostgreSQL para grupo com retenção de 14 dias; monitore o volume. CloudTrail de conta não é substituído por este projeto.
- AMI AL2023 otimizada ECS vem do SSM. Atualizar o launch template afeta novas instâncias; planeje rotação/drenagem dos hosts existentes para aplicar patches. Não há rotina automática de patch/reboot nesta entrega.
- Limite `max_tasks=4` protege o RDS micro: até 8 tasks durante rollout x 4 conexões = 32, mais migration/admin abaixo das 50 configuradas. Redimensione banco/pools antes de aumentar. Testes de carga são necessários para dimensionar usuários/IA.
...
Os testes usam `mock_provider`: os `apply` dentro de testes **não chamam AWS nem criam recursos**. Cobrem dimensões, bootstrap, banco privado/criptografado, contêiner, IMDS e variante privada/HA. Não substituem um deploy real, validação das permissões IAM na conta ou teste ponta a ponta da CDN. Veja [custos mensais](CUSTOS.md).
